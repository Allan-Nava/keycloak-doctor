...
- **Gate prima di chiudere**: `go vet ./...` + `go test ./...` + `golangci-lint run` + `go run ./cmd/gen-docs` (deve essere un no-op) verdi. Serve **golangci-lint v2** (config schema v2): `go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.12.2`, stessa versione pinnata in CI. **Va reinstallato dopo ogni bump del Go locale**: un binario compilato con go1.N va in panic caricando la stdlib di go1.N+1 (`file requires newer Go version`) — è già capitato due volte, con 1.25→1.26 e 1.26→1.27. Per lo stesso motivo la CI lo compila da sorgente con il toolchain del job invece di scaricare il binario pre-compilato.
...
- `internal/output/` — renderer: `Text` (terminale), `Markdown` (report/PR comment), `JSON` (contratto di gating, ed è ciò che `--baseline` rilegge), `SARIF` (code scanning). I renderer vedono solo finding, mai il modello del realm: il catalogo per il descrittore SARIF arriva da `Options.Rules`, non da un import di `internal/rules`.
- `internal/suppress/` — file di suppression: entry con `until` e `reason` **obbligatori**, matching su rule/realm/target, e i finding `suppression/expired` / `suppression/unmatched` sul file stesso. Non sono regole del catalogo (non sono funzioni del realm): non entrano in `rules.All()` e `--only`/`--skip` non le selezionano, sono documentate in `docs/ci.md`.
...
- **`og:image` deve essere un raster**: nessun crawler e nessun client di chat renderizza SVG, quindi la card è un PNG 1200×630 **committato**, non generato in CI (il testo della card è testo SVG, reso con i font della macchina che lo renderizza — in CI non sono gli stessi). Il comando per rigenerarla è in `docs/brand.md`.
- **Il SEO dipende da `baseURL` in `cmd/gen-site`**: canonical, `og:url`, `og:image` e la sitemap sono assoluti e vengono da lì. Senza `BaseURL` il generatore **non** scrive né sitemap né robots e omette quei tag, invece di emetterne di rotti: mezza sitemap è peggio di nessuna sitemap.
...
- **`--fail-on-new` non crea un gate, lo restringe**: richiede `--baseline` (senza, ogni finding è nuovo) e `--exit-on` (non c'è niente da restringere). Sono errori d'uso, non default: indovinare qui vuol dire indovinare se la pipeline di qualcuno fallisce.
- **Ordine dentro un run**: audit → suppress → confronto col baseline → `--min-severity` → render → gate. Un finding soppresso non deve arrivare al confronto (comparirebbe come "risolto" e poi di nuovo come "nuovo" alla scadenza) e non può soddisfare un gate, perché non è più nel run.
- **"Non lo so" non è "OK"**: in SARIF un finding `ERROR` è un result a livello `note` (non una regola saltata), e negli output dell'action `worst` diventa `UNKNOWN` se l'audit non è rileggibile. Un punto cieco non deve mai renderizzare come esito pulito — vale nel terminale, in SARIF e negli output di un job.
...
- CI di chi usa il tool: `action.yml` (composite: scarica il binario di release e verifica il checksum; `version: path` usa un binario già sul PATH, ed è come la CI testa l'action sul working tree) · `docs/ci.md`
...
- Brand: `docs/brand.md` · Logo: `docs/assets/logo.svg` + `docs/assets/favicon.svg` · Card dei link preview: `docs/assets/og-card.svg` (sorgente) → `og-card.png` (committata) — unica copia, li leggono sia il README sia `cmd/gen-site` (che li serve accanto alle pagine). Il segno è shield + keyhole + pulsazione: **niente checkmark**, il tool non dà verdetti.
